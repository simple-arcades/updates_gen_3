...
# Level B permissions:
# - If target file/dir already exists, preserve its owner/group/mode
# - If target is new, inherit owner/group from nearest existing parent dir
# - New items keep the mode from the update pack by default
# - Explicit FILE_PERMISSIONS overrides always win (for special cases like read-only)
...
# Recommended ON (requires publishing *.sha256 files)
CHECKSUM_VERIFY_ENABLED=1

# Customer WiFi is often flaky
WGET_TIMEOUT=20
WGET_TRIES=3
...
# ----------------------------
# Only add entries here when you NEED special behavior.
# Example for read-only files:
# ["/home/pi/RetroPie/roms/atomiswave/somefile.bin"]="root:root 444"
...
validate_tarball_safe() {
  local archive="$1"
  local entry
  # Reject absolute paths and any ".." traversal inside tar
...
verify_checksum_for_download() {
  local filename="$1"   # e.g. 11.1.8.tar.gz or update_system.sh
...
# ----------------------------
# Level B permissions engine
# ----------------------------

# Find nearest existing parent dir for a path, return numeric uid:gid
nearest_parent_og() {
  local p="$1"
  while [ "$p" != "/" ] && [ ! -d "$p" ]; do
    p="$(dirname "$p")"
...
# Build a permission plan for everything in the update pack (excluding META_DIR_NAME and control files).
# For each path we record:
# type (D/F) | relpath | existed_before (0/1) | old_uidgid | old_mode | staged_mode
build_perm_plan() {
  local root="$1"
  local plan="$2"
  : > "$plan"
...
# Apply the permission plan after rsync + deletes.
apply_perm_plan() {
  local plan="$1"
  ...
  awk -F'\t' '$1=="D"{print}' "$plan" | while IFS=$'\t' read -r typ rel existed oldog oldmode stagedmode; do
    ...
    if [ "$existed" -eq 1 ] && [ -n "$oldog" ] && [ -n "$oldmode" ]; then
      ...
      # New dir: inherit owner/group from nearest existing parent, keep staged mode
  ...
  awk -F'\t' '$1=="F"{print}' "$plan" | while IFS=$'\t' read -r typ rel existed oldog oldmode stagedmode; do
    ...
    if [ "$existed" -eq 1 ] && [ -n "$oldog" ] && [ -n "$oldmode" ]; then
      ...
      # New file: inherit owner/group from nearest existing parent, keep staged mode
      dparent="$(dirname "$target")"
      parentog="$(nearest_parent_og "$dparent")"
      chown "$parentog" "$target" >/dev/null 2>&1
      [ -n "$stagedmode" ] && chmod "$stagedmode" "$target" >/dev/null 2>&1
...
self_update_if_needed() {
  ...
  # If checksums enabled, require update_system.sh.sha256
...
apply_updates() {
  ...
  (
    ...
    for update in "${updates[@]}"; do
      ...
      # Build permission plan BEFORE applying, so we can preserve existing perms
      ...
      # Apply Level B permissions now (skip missing paths)
      apply_perm_plan "$perm_plan"
      rm -f "$perm_plan" >/dev/null 2>&1

      # Apply explicit overrides last (read-only rules, etc.)
      apply_file_permission_overrides
