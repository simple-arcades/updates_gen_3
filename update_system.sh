...
# SCRIPT_BUILD=2026-01-16-gen3-v6.1 (fix: CRLF/BOM-safe checksums + no nested dialogs during gauge)
# Design goals:
...
show_message() {
  ...
  # Don't open nested dialogs while a progress gauge is running; it can freeze the UI.
  if [ "${IN_GAUGE:-0}" = "1" ]; then
    log_update "(gauge) $message"
    return 0
  fi

  # If we're not on a real TTY (non-interactive run), don't try to use dialog.
  if ! [ -t 1 ]; then
    echo "$message"
    return 0
  fi
...
verify_checksum_for_download() {
  ...
  # Normalize checksum file so it works even if created on Windows (CRLF) or with a UTF-8 BOM.
  # (CRLF breaks sha256sum -c because it looks for a filename ending in "\r".)
  if [ -f "$checksum_path" ]; then
    tr -d '\r' < "$checksum_path" > "$checksum_path.tmp" 2>/dev/null && mv -f "$checksum_path.tmp" "$checksum_path"
    LC_ALL=C sed -i '1s/^\xEF\xBB\xBF//' "$checksum_path" 2>/dev/null || true
  fi
...
apply_perm_plan() {
  ...
  awk -F'\t' '$1=="D"{print}' "$plan" | while IFS=$'\t' read -r typ rel existed oldog oldmode stagedmode; do
    ...
    if [ "$existed" -eq 1 ] && [ -n "$oldog" ] && [ -n "$oldmode" ]; then
      ...
      # New file: inherit owner/group from nearest parent, and derive a sane mode from the parent directory.
# We intentionally DO NOT trust modes stored in the update tarball (common Windows/WSL tar issue -> 0777).
local pmode fmode
pmode="$(stat -c '%a' "$dparent" 2>/dev/null)"
# default file mode is parent_mode masked to rw bits (no execute): e.g., 755 -> 644, 775 -> 664, 777 -> 666
if [ -n "$pmode" ]; then
...
chmod "$fmode" "$target" >/dev/null 2>&1
    fi
  done
